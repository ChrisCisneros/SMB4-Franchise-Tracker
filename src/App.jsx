...
export default function App() {
  ...
  useEffect(() => {
    if (!hasLoadedFirebaseTeams.current) return;
    const firebaseTeamsRef = ref(db, "teams");
    set(firebaseTeamsRef, data.teams).catch(() => {
      // ignore for now
    });
  }, [data.teams]);

  useEffect(() => {
    if (!hasLoadedFirebasePlayers.current) return;
    const firebasePlayersRef = ref(db, "players");
    set(firebasePlayersRef, data.players).catch(() => {
      // ignore for now
    });
  }, [data.players]);
